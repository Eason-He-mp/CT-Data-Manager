...
class CTDataApp:
    ...
    def process_single_engineer(self, base_dir, engineer_name, threshold_bytes, cutoff_date, nasuni_pattern):
        ...
        for project_name in items_in_eng_dir:
            ...
            # 2. 包含 FACT 但未上传 (Waiting to upload)
            elif "FACT" in project_name:
                category = "Waiting to Upload"
    ...
    def show_details(self, engineer_name):
        ...
        if info['waiting_projects']:
            ...
            waiting_node = tree.insert("", tk.END, text=waiting_title, values=(
                "Group", "-", "-", format_size(info['waiting_total_size'])
            ), tags=('group_node',))
            ...
            for proj in sorted_waiting:
                tree.insert(waiting_node, tk.END, text=f"📁 {proj['name']}", values=(
                    proj['category'], proj['age'], proj['date'], proj['size_str']
                ))
        ...
        if info['nasuni_projects']:
            ...
            nasuni_node = tree.insert("", tk.END, text=nasuni_title, values=(
                "Group", "-", "-", format_size(info['nasuni_total_size'])
            ), tags=('group_node',))
